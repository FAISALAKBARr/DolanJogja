...
var Stamen_Watercolor = L.tileLayer(
  ...
  {
    attribution:
      'Map tiles by <a href="http://stamen.com">Stamen Designs</a>, <a href="http://creativecommons.org/licenses/by/3.0">CC BY 3.0</a> &mdash; Map data &copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors',
    subdomains: "abcd",
    minZoom: 1,
    maxZoom: 16,
    ext: "jpg",
  ...
  while (openSet.length > 0) {
    ...
    for (const neighbor in graph[current].neighbors) {
      const tentativeGScore =
        gScore[current] + graph[current].neighbors[neighbor];
      if (tentativeGScore < (gScore[neighbor] || Infinity)) {
        cameFrom[neighbor] = current;
        gScore[neighbor] = tentativeGScore;
        fScore[neighbor] =
          tentativeGScore +
          heuristic(graph[neighbor].coords, graph[end].coords);
